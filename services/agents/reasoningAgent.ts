...
// ══════════════════════════════════════════════════════════════════
// SEDREX — Reasoning Agent v2.0
//
// TODAY : Gemini 2.5 Pro with deep reasoning unlock + analytical
//         domain protocol. Extracts maximum reasoning capability.
...
// Uses buildAgentSystemPrompt which injects:
//   Identity lock + Core Intelligence + Reasoning domain +
//   Gemini deep reasoning unlock + long context unlock (if needed)
export function buildReasoningSystemPrompt(
  sessionContext?: string,
  hasLongContext = false,
): string {
  return buildAgentSystemPrompt("reasoning", {
    sessionContext,
    hasLongContext,
  });
}
...
export async function callReasoningAgent(
  ...
  if (PROVIDER_CLAUDE.available) {
    console.log("[SEDREX Agent] Reasoning → Claude Sonnet");
    ...
    if (onStreamChunk && res.body) {
      ...
      return { text: fullText, inputTokens, outputTokens, provider: "claude", agentType: "reasoning" };
    }
    ...
    return {
      ...
      provider: "claude", agentType: "reasoning",
    };
  }

  // ── Gemini fallback ───────────────────────────────────────────────
  // Returns empty → orchestrator injects reasoning system prompt
  // into Gemini call in aiService.ts. Full deep reasoning active today.
  console.log("[SEDREX Agent] Reasoning → Gemini 2.5 Pro (VITE_CLAUDE_KEY not set)");
  return { text: "", inputTokens: 0, outputTokens: 0, provider: "gemini-fallback", agentType: "reasoning" };
}